...
from .archetypal_dictionary import RelaxedArchetypalDictionary, SoftmaxArchetypalDictionary
...
class RATopKSAE(TopKSAE):
    ...
    **kwargs : dict, optional
    ...
    def __init__(self, input_shape, nb_concepts, points, top_k=None, delta=1.0, use_multiplier=True, projection: bool=True, **kwargs):
...
class SoftmaxRATopKSAE(TopKSAE):
    """
    Softmax Archetypal TopK SAE.

    Variant of RATopKSAE where the convex combination weights are parameterized
    via softmax instead of a projected gradient. This makes the dictionary fully
    differentiable with no no_grad blocks or in-place .data mutations, allowing
    Adam to maintain consistent moment estimates.
    ...
    points : torch.Tensor
        Data points used to define archetypes, shape (nb_candidates, input_shape).
    top_k : int, optional
        Number of top activations to keep. Defaults to 10% sparsity.
    use_multiplier : bool, optional
        Whether to train a positive scalar multiplier, by default True.
    **kwargs : dict, optional
        Additional arguments passed to TopKSAE.
    """

    def __init__(self, input_shape, nb_concepts, points, top_k=None,
                 use_multiplier=True, **kwargs):
        assert isinstance(input_shape, int), "SoftmaxRATopKSAE input_shape must be an integer."

        super().__init__(input_shape=input_shape, nb_concepts=nb_concepts,
                         top_k=top_k, **kwargs)

        self.dictionary = SoftmaxArchetypalDictionary(
            in_dimensions=input_shape,
            nb_concepts=nb_concepts,
            points=points,
            use_multiplier=use_multiplier,
